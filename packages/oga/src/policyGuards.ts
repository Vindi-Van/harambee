...
export function requiresHumanApprovalForPr(
  ...
  // Only apply PR checks for transitions that involve PRs
  const isPrTransition =
    (request.from === "execution" && request.to === "verification") ||
    (request.from === "verification" && request.to === "deployment");
